...
export function Game() {
  ...
  const [isGameOver, setGameOver] = useState(false); // Thêm trạng thái kết thúc trò chơi
  ...
  const updateGameMatrix = (column: number, row: number, symbol: "x" | "o") => {
    if (isGameOver) return; // Ngừng trò chơi khi đã kết thúc
    ...
    if (newMatrix[row][column] === null) {
      ...
      if (socketService.socket) {
        ...
        setPlayerTurn(false);  // Đặt lượt người chơi là false sau khi có kết quả
      }
    }
  };
  ...
  const handleGameStart = () => {
    if (socketService.socket)
      gameService.onStartGame(socketService.socket, (options) => {
        setGameStarted(true);
        setPlayerSymbol(options.symbol || "x");
  ...
  return (
    <GameContainer>
      ...
      {(!isGameStarted || !isPlayerTurn || isGameOver) && <PlayStopper />}
