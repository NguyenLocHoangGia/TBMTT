...
export function Game() {
  ...
  const [isGameOver, setGameOver] = useState(false); // Trạng thái kết thúc trò chơi

  const {
    playerSymbol,        // Biểu tượng của người chơi ("x" hoặc "o")
    setPlayerSymbol,     // Hàm cập nhật biểu tượng
    setPlayerTurn,       // Hàm đặt lượt chơi
    isPlayerTurn,        // Trạng thái lượt chơi
    setGameStarted,      // Hàm đặt trạng thái bắt đầu trò chơi
    isGameStarted,       // Trạng thái trò chơi
  ...
  const checkGameState = (matrix: IPlayMatrix): [boolean, boolean] => {
    ...
    if (matrix.every((row) => row.every((cell) => cell !== null))) return [true, true];
  ...
  const resetGame = () => {
    ...
    setGameOver(false);
    setPlayerTurn(false); // Đặt lượt chơi về trạng thái ban đầu
    setGameStarted(false);

    // Gửi yêu cầu bắt đầu lại trò chơi
    if (socketService.socket) {
      gameService.startNewGame(socketService.socket);
    }
  };

  // Cập nhật ma trận khi người chơi click
  const updateGameMatrix = (column: number, row: number, symbol: "x" | "o") => {
    if (isGameOver || !isPlayerTurn) return; // Chặn click nếu trò chơi đã kết thúc hoặc không đến lượt
    ...
    if (newMatrix[row][column] === null) {
      ...
      if (socketService.socket) {
        ...
        setPlayerTurn(false); // Chuyển lượt cho đối phương
      }
    }
  };
  ...
  useEffect(() => {
    if (socketService.socket) {
      gameService.onGameUpdate(socketService.socket, (newMatrix) => {
        setMatrix(newMatrix);
        setPlayerTurn(true); // Đặt lại lượt chơi
      });
      ...
      gameService.onStartGame(socketService.socket, (options) => {
        setGameStarted(true);
        setPlayerSymbol(options.symbol); // Phân biệt X và O từ server
